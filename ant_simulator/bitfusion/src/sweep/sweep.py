                                    ...
                                    for obuf in list_obuf:
                                        ...
                                        for bw in list_bw:
                                            ...
                                            for b in list_bench:
                                                ...
                                                lookup_dict["N"] = n
                                                lookup_dict["M"] = m
                                                lookup_dict["Max Precision (bits)"] = (
                                                    pmax
                                                )
                                                lookup_dict["Min Precision (bits)"] = (
                                                    pmin
                                                )
                                                ...
                                                if bench_type == "ant":
                                                    nn = benchmarks.get_bench_nn_ant(
                                                        b, batch_size
                                                    )
                                                if bench_type == "ant_weight":
                                                    nn = benchmarks.get_bench_nn_ant_weight(
                                                        b, batch_size
                                                    )
                                                elif bench_type == "bit":
                                                    nn = benchmarks.get_bench_nn_bit(
                                                        b, batch_size
                                                    )
                                                elif bench_type == "ada":
                                                    nn = benchmarks.get_bench_nn_ada(
                                                        b, batch_size
                                                    )
                                                elif bench_type == "ola":
                                                    nn = benchmarks.get_bench_nn_ola(
                                                        b, batch_size
                                                    )
                                                elif bench_type == "bis":
                                                    nn = benchmarks.get_bench_nn_bis(
                                                        b, batch_size
                                                    )

                                                if len(results) == 0:
                                                    self.logger.info(
                                                        "Simulating Benchmark: {}".format(
                                                            b
                                                        )
                                                    )
                                                    self.logger.info(
                                                    ...
                                                    self.logger.info(
                                                        "Max Precision (bits): {}".format(
                                                            pmax
                                                        )
                                                    )
                                                    self.logger.info(
                                                        "Min Precision (bits): {}".format(
                                                            pmin
                                                        )
                                                    )
                                                    self.logger.info(
...
def check_pandas_or_run(
    ...
    if len(results) == 0:
        ...
        dataframe = sweep_obj.sweep(
            sim,
            list_bench=list_bench,
            list_batch=[batch_size],
